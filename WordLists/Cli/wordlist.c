// System dictionaries  generated by parsewords.py  Feb 27, 2017  21:29:41
...
//	khz kilohertz // ( n ) set frequency in Khz for DAC 1 output
	NAME("ackcmd")		//  send ack active command
	NAME("idcmd")		//  send request for sensor id
	NAME("ackrsp")		//  send ack active reponse 
	NAME("sa")		//  ( a s ) set sensor s address to a
	NAME("da")		//  ( a ) set device to address a
	NAME("sp1")		//  ( d )  send data byte out port 1
	NAME("sp2")		//  ( d )  send data byte out port 2
	NAME("sp4")		//  ( d )  send data byte out port 4
	NAME("sm")		//  ( f )  turn monitoring on 1 or off 0 - default on
	NAME("mm")		//  ( f n s ) set measurement n of sensor s to float f
	NAME("gm")		//  ( n ) get measurement n
	NAME("sensors")		//  list all sensors and details
	NAME("sd1")		//  ( b ) set sense detect connect to 1 on, 0 off
	NAME("sd2")		//  ( b ) set sense detect connect to 1 on, 0 off
END_NAMES
...
void senseDetect1(void);
void senseDetect2(void);
